...
..core.colorize("#FFFFFF", "Enemy penetration: 15%\n") ..core.colorize("#FFFFFF", "Bullet velocity: 40"),
	range = 0,
	inventory_image = "rangedweapons_ak47.png",
	on_pickup = function(item, lname, table)
	
		
			--local dropper = lname
			pname = lname:get_player_name()
			inv = lname:get_inventory()
			local pos = lname:get_pos()
			--print(hard.ifp.player[pname].arm.weapon_item)
			
			if pname and inv then
				central.save_state_arm(pname, false, "rangedweapons:ak47")
				inv = lname:get_inventory()
				inv:add_item("main", ItemStack("rangedweapons:ak47"))
	...
	--[[on_drop = function(itemstack, dropper, pos)
		...
		hard.ifp.player[name].arm = {weapon_string = nil, weapon_item = nil}
		inv:remove_item("main", ItemStack("rangedweapons:ak47"))
	end,--]]
	RW_gun_capabilities = {
		automatic_gun = 1,
